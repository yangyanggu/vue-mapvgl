const THREE = require('mapvgl/dist/mapvgl.threelayers.min').THREE;
...
let GLTFLoader = (function() {
  ...
  GLTFLightsExtension.prototype._markDefs = function() {
    ...
    let nodeDefs = this.parser.json.nodes || [];
  ...
  GLTFLightsExtension.prototype._loadLight = function(lightIndex) {
    ...
    let parser = this.parser;
    let cacheKey = 'light:' + lightIndex;
    let dependency = parser.cache.get(cacheKey);

    if (dependency) return dependency;

    let json = parser.json;
    let extensions = (json.extensions && json.extensions[ this.name ]) || {};
    let lightDefs = extensions.lights || [];
    let lightDef = lightDefs[ lightIndex ];
    let lightNode;
  ...
  GLTFLightsExtension.prototype.createNodeAttachment = function(nodeIndex) {
    ...
    let lightDef = (nodeDef.extensions && nodeDef.extensions[ this.name ]) || {};
    let lightIndex = lightDef.light;
  ...
  GLTFTextureWebPExtension.prototype.loadTexture = function(textureIndex) {
    ...
    let source = json.images[ extension.source ];
    let loader = source.uri ? parser.options.manager.getHandler(source.uri) : parser.textureLoader;
  ...
  */
  ...
  GLTFMeshoptCompression.prototype.loadBufferView = function(index) {
    ...
    let bufferView = json.bufferViews[ index ];

    if (bufferView.extensions && bufferView.extensions[ this.name ]) {

      let extensionDef = bufferView.extensions[ this.name ];

      let buffer = this.parser.getDependency('buffer', extensionDef.buffer);
      let decoder = this.parser.options.meshoptDecoder;
      ...
      return Promise.all([ buffer, decoder.ready ]).then(function(res) {
        ...
        let byteLength = extensionDef.byteLength || 0;

        let count = extensionDef.count;
        let stride = extensionDef.byteStride;

        let result = new ArrayBuffer(count * stride);
        let source = new Uint8Array(res[ 0 ], byteOffset, byteLength);
    ...
    if (source.bufferView !== undefined) {
      ...
      sourceURI = parser.getDependency('bufferView', source.bufferView).then(function(bufferView) {
        ...
        if (source.mimeType === 'image/png') {
          ...
          let colorType = new DataView(bufferView, 25, 1).getUint8(0, false);
  ...
  GLTFParser.prototype.loadNode = function(nodeIndex) {
    ...
    let nodeName = nodeDef.name ? parser.createUniqueName(nodeDef.name) : '';
